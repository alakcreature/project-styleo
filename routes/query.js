...
router.post('/', (req, res) => {
    ...
    var q2 = b.find({ name: req.body.figurename }).select('field name avoid use optional').exec();
    var q3 = c.find({ name: req.body.skinname }).select('field name avoid use').exec();
    var q4 = d.find({ name: req.body.heightname }).select('field name avoid use').exec();
